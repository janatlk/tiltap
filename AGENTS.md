...
|----------|---------------|----------------|----------------|-------|
| `ky` | Whisper `nineninesix/kyrgyz-whisper-small` converted to CTranslate2 on RunPod GPU | Vosk `vosk-model-ky-0.42` (CPU, if GPU fails/times out) | 68.1% / 29.5% | The GPU worker patches faster-whisper to accept the custom `<|ky|>` token and decodes with `beam_size=1`, `condition_on_previous_text=False`, `without_timestamps=True`, `temperature=0.0`, **no n-gram blocking** (`KYRGYZ_NO_REPEAT_NGRAM_SIZE=0` — blocking legitimate Kyrgyz repetitions cost ~16 char points) and **fixed 15 s chunks** (`GPU_VAD_ENABLED=false` — Silero VAD splits also cost quality; verified 2026-07-15). A post-decode byte-token cleanup step fixes any raw UTF-8 byte leakage from the model. CPU fallback is automatic when the GPU worker fails or is disabled. |
