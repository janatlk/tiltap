...
**Что реально происходит на бою** (замерено 2026-08-06, не выводите из дефолтов выше):
на проде `TILTAB_GPU_STT_LANGUAGES=en,auto,multi` — список сужен вручную, `tg` в него
не входит. Поэтому:

| язык | движок | скорость | час аудио |
|---|---|---|---|
| `en` | RunPod GPU (RTX A4500) | — | быстро |
| `ky`, `uz`, `ru` | GigaAM через `gigaam_server.py`, модель резидентна | 24.6x | ~2.4 мин |
| `tg` | whisper-tajik-finetuned-ct2 на CPU, модель перезагружается | 1.9x | **~32 мин** |

Таджикский — единственный язык на CPU и в 13 раз медленнее киргизского. Час таджикского
видео занимает полчаса процессора. Тот же замер через spawn для `ru` даёт 9.6x против
24.6x у резидентной модели, то есть 2.5x теряется только на перезагрузке весов.

GPU-воркер везёт для `tg` отдельную модель `muhtasham-whisper-tg-ct2` — это **не** та же
модель, что локальная `whisper-tajik-finetuned-ct2` (одинаковый размер, разные веса).
Что из них точнее — не измерено. Перед переносом `tg` на GPU это надо сравнить, иначе
меняем качество на скорость вслепую.

Распознавание сериализовано шлюзом в `concurrencyGate.ts` (одно за раз), скачивания —
не больше четырёх. Замер: две задачи параллельно 13.1с против 14.3с последовательно,
то есть параллелить смысла нет, GigaAM и так занимает все ядра.
