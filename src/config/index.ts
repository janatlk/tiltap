...
const envSchema = z.object({
  ...
  TILTAB_GIGAAM_SERVER_LANGUAGES: z
    ...
    .optional()
    .or(z.literal(""))
    // Русского здесь больше нет: на смешанной русско-английской записи
    // GigaAM пишет английские вставки кириллицей ("гуд монин", "вотсап") и
    // не ставит знаков препинания, а Whisper даёт готовый к чтению текст при
    // той же точности на самом русском. Замер: см. коммит с этой правкой.
    .default("ky,uz")
    .transform((val) =>
      val
        ?.split(",")
  ...
  // Потолок на объём одного документа, в токенах. Считаем примерно два
  // символа на токен (кириллица), то есть 60 000 токенов — это около 120 тысяч
  // знаков, примерно шестьдесят страниц. Такой документ переводится минуты за
  // полторы и стоит около десяти центов; книга на тысячу страниц, присланная
  // по ошибке, не пройдёт.
  TILTAB_MAX_DOCUMENT_TOKENS: z.string().default("60000").transform(Number),
