#!/usr/bin/env python3
"""Quick language detection test using distil-large-v3 with faster-whisper."""
import json
import os
import sys
import tempfile
...
def detect_language(input_path: str) -> str:
    ...
    try:
        ...
        # Force the generator to start so info.language is populated
...
for label, path in files.items():
    if not os.path.exists(path):
        ...
        continue
    print(f"[detect] {label} ...", file=sys.stderr, flush=True)
    try:
        results[label] = detect_language(path)
